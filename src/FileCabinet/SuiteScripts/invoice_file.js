...
define(['N/file', 'N/search', 'N/log'], function(file, search, log) {
    ...
    function map(context) {
        ...
        try {
            ...
            if (Array.isArray(fileDescriptionArray)) {
                // Check if the object already exists in the description array
                var objectExists = fileDescriptionArray.some(function(descObj) {
                    return JSON.stringify(descObj) === JSON.stringify(fileDescriptionObj); // check for alterate for object comparison
                });
            ...
            loadedFile.description = JSON.stringify(fileDescriptionArray);

            // Set the checkbox "Available for Suitebundles" to true
            loadedFile.bundleable = true;
