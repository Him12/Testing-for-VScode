...
define(['N/record', 'N/log', 'N/runtime', 'N/search'], function (record, log, runtime, search) {
  ...
  var nsIds = {
    ...
    oppSalesProcess_InstallType: 'custbody.Install_type',
  ...
  function beforeLoad(context) {
    try {
      ...
      log.debug({
        title: 'beforeLoad',
        details: `Event Type: ${context.type}, Record ID: ${newRecord.id}, Record Type: ${newRecord.type}`
      });

      // VIEW mode: Handle both Forecast and SFA Forecast subtab updates
      if (context.type === context.UserEventType.VIEW) {
        var isFormValid = isFormMatching(context);

        // Forecast subtab logic (only if form matches)
        if (isFormValid) {
          var salesProcesses = _calculateSalesProcessQuestionTotals(context);
          var oppTotals = _getOppForecastSubtabTitle(context); // Forecast subtab amount

          log.debug('Sales Processes', salesProcesses);
          log.debug('Opportunity Totals', oppTotals);

          var scriptTextForecast = '<scr' + 'ipt>' +
          ...
          var fieldForecast = form.getField(nsIds.viewModeScriptField);
          if (fieldForecast) {
            fieldForecast.defaultValue = scriptTextForecast;
            log.debug('Script Text set to Forecast field', scriptTextForecast);
          } else {
            log.error('Forecast Field not found', 'Field ID: ' + nsIds.viewModeScriptField);
          }
        }

        // SFA Forecast subtab logic (no form matching check, as it’s working)
        var opportunityValue = newRecord.getValue(nsIds.oppProjectedTotal) || 0;
        log.debug({
        ...
        var forecastSearch = search.create({
          ...
          filters: [[nsIds.oppForecast_relatedOpp, search.Operator.IS, newRecord.id]],
        ...
        var scriptTextSFA = '<scr' + 'ipt>' +
          'var Helpers = {};' +
          'Helpers.formatNumber = ' + String(Helpers.formatNumber) + ';' +
          'Helpers.updateSubtabTitle = ' + String(Helpers.updateSubtabTitle) + ';' +
          'var _updateOppSFAForecastSubtabTitle = ' + String(_updateOppSFAForecastSubtabTitle) + ';' +
          'jQuery(document).ready(function() {' +
          '_updateOppSFAForecastSubtabTitle(\'' + totalForecastAmount.replace(/\'/g, "\\'") + '\');' +
          '});' +
          '</scr' + 'ipt>';

        var fieldSFA = form.getField(nsIds.viewModeSFAScriptField);
        if (fieldSFA) {
          fieldSFA.defaultValue = scriptTextSFA;
          log.debug('Script Text set to SFA field', scriptTextSFA);
        } else {
          log.error('SFA Field not found', 'Field ID: ' + nsIds.viewModeSFAScriptField);
        }

        // Hide oppCompetitionOther field if not applicable
        if (newRecord.getValue(nsIds.oppCompetition) !== nsVals.competition.other) {
          var compField = form.getField(nsIds.oppCompetitionOther);
          if (compField) {
            compField.updateDisplayType({ displayType: 'hidden' });
          }
        }
      }

      // EDIT and CREATE mode: Market Sector field logic
  ...
  var Helpers = (function () {
    function _updateSubtabTitle(subtabTitle, newTitleId, newTitle) {
      ...
      for (var i = 0; i < formTabs.length; i += 1) {
    ...
    salesProcesses.forEach(function (e, i) {
      e.fields.forEach(function (fieldName) {
        if (record.getField(fieldName)) {
          ...
          switch (fieldType) {
            case "checkbox":
              if (!record.getField(fieldName).isHidden) {
  ...
  function _getOppForecastSubtabTitle(context) {
    if (context.newRecord.id) {
      ...
      for (var i = 0; i < searchResult.length; i++) {
        ...
        totalAmount += parseFloat(f.getValue(amount)) || 0;
      }
  ...
  function updateStartDates(tranName, tranId, newDateValue) {
    ...
    switch (tranName) {
      case 'opportunity':
        try {
          ...
          newDate = newDateValue || opportunityRecord.getValue({ fieldId: nsIds.expectedStartDate });
        } catch (e) {
          log.error('Error loading opportunity record', e.message);
        }
        break;
      case 'estimate':
        try {
          ...
          newDate = newDateValue || estimateRecord.getValue({ fieldId: nsIds.expectedStartDate });
        } catch (e) {
          log.error('Error loading estimate record', e.message);
        }
        break;
      case 'salesOrder':
        try {
          ...
          newDate = newDateValue || salesOrderRecord.getValue({ fieldId: 'startdate' });
          foundTransactions.push({
            id: salesOrderRecord.getValue({ fieldId: 'createdfrom' }),
            tranType: 'Estimate'
          });
        } catch (e) {
          log.error('Error loading sales order record', e.message);
        }
        break;
      default:
        break;
    ...
    log.audit('_updateStartDates', tranName + ', ' + tranId + ': ' + JSON.stringify(oldDate) + ' === ' + JSON.stringify(newDate) + ' ? ' + (oldDate === newDate));
    ...
    if (tranName === 'opportunity') {
      ...
      var oppId = context.newRecord.getValue({ fieldId: 'opportunity' });
      if (oppId) {
        foundTransactions.push({
          id: oppId,
    ...
    foundTransactions.forEach(function (tran) {
      if (tran.id) {
        try {
          ...
          log.error('Error updating transaction', e.message);
        }
      }
    });
  }
