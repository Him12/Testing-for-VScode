            ...
            try {
                ...
                if (context.searchType == "job") {
                    ...
                    if (jobColumns && jobFilters) {
                        ...
                        // Log the columns of the saved search to verify they include the expected fields
                        log.debug("Saved Search Columns", JSON.stringify(jobSearchObj.columns));
                        ...
                        var jobResults = [];
                        var jobDict = {}; // Object to group work types by job internal ID

                        // Process results in batches of 1,000
                        var pagedData = jobSearchObj.runPaged({ pageSize: 1000 });
                        log.debug("Total Job Search Results:", pagedData.count);

                        pagedData.pageRanges.forEach(function (pageRange) {
                            var page = pagedData.fetch({ index: pageRange.index });
                            page.data.forEach(function (result) {
                                var internalId = result.getValue({ name: "internalid" });
                                var jobId = result.getValue({ name: "entityid" });
                                var workTypeId = result.getValue({ name: "internalid", join: "CUSTRECORD_OSM_RELATIONSHIP_JOB" }) || "";
                                var workTypeDescription = result.getValue({ name: "name", join: "CUSTRECORD_OSM_RELATIONSHIP_JOB" }) || "";
                                var isInactiveRaw = result.getValue({ name: "isinactive", join: "CUSTRECORD_OSM_RELATIONSHIP_JOB" });

                                // Enhanced debugging: Log all relevant values for each row
                                log.debug("Work Type Debug", "internalId: " + internalId + ", jobId: " + jobId + ", workTypeId: " + workTypeId + ", workTypeDescription: " + workTypeDescription + ", isinactive: " + isInactiveRaw + " (type: " + typeof isInactiveRaw + ")");

                                // Handle isinactive value with fallback
                                var isInactive;
                                if (isInactiveRaw === null || isInactiveRaw === undefined || isInactiveRaw === "") {
                                    log.debug("isinactive Missing", "No isinactive value for workTypeId: " + workTypeId + "; defaulting to not inactive (no)");
                                    isInactive = "no"; // Default to not inactive if isinactive is not available
                                } else if (typeof isInactiveRaw === "boolean") {
                                    isInactive = isInactiveRaw ? "yes" : "no"; // Boolean: true means inactive ("yes"), false means not inactive ("no")
                                } else {
                                    // String: "T" means inactive ("yes"), "F" means not inactive ("no")
                                    isInactive = (isInactiveRaw === "T" || isInactiveRaw === "true" || isInactiveRaw === true) ? "yes" : "no";
                                }

                                // Log the computed isInactive value with expected mapping
                                log.debug("Computed isInactive", "workTypeId: " + workTypeId + ", workTypeDescription: " + workTypeDescription + ", isInactiveRaw: " + isInactiveRaw + ", isInactive: " + isInactive + " (Expected: 'T' or true = inactive ('yes'), 'F' or false = not inactive ('no'))");

                                // If the job is already in the dictionary, add the work type to its list
                                if (jobDict.hasOwnProperty(internalId)) {
                                    var jobEntry = jobDict[internalId];
                                    if (workTypeId) { // Only add work type if it exists
                                        jobEntry.workType.push({
                                            workTypeId: workTypeId,
                                            workTypeDescription: workTypeDescription,
                                            workTypeInactive: isInactive
                                        });
                                    }
                                } else {
                                    // Otherwise, create a new job entry with the work type
                                    jobDict[internalId] = {
                        ...
                        // Convert the dictionary to an array for the final result
                        for (var key in jobDict) {
                            if (jobDict.hasOwnProperty(key)) {
                                jobResults.push({
                                    internalId: jobDict[key].internalId,
                                    jobId: jobDict[key].jobId,
                                    workType: jobDict[key].workType
                                });
                            }
                        }

                        // Log the final result for verification
                        log.debug("Final Job Results", JSON.stringify(jobResults));

                        return { jobs: jobResults };
                    } else {
                        return JSON.stringify({ error: "Columns and filters are required for Job search" });
                    }
                }
                else
