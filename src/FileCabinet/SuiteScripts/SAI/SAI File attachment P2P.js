...
* with Symphoni.
*
* Script Name: SAI | MR P2P file attachment.js
...
define(['N/search', 'N/record', 'N/log'], function(search, record, log) {
    ...
    function getInputData() {
        var customrecord_record_attachmentSearchObj = search.create({
            type: "customrecord_sai_p2p_attachment",
            filters:
            [
               ["custrecord_sai_p2p_external_id","isnotempty",""]
               //"AND",
               //["internalidnumber","equalto","252"]
            ],
            columns:
            [
               search.createColumn({name: "custrecord_sai_p2p_external_id", label: "External Id"}),
               search.createColumn({name: "custrecord_sai_p2p_file_attachment", label: "Attachment"}),
               search.createColumn({name: "custrecord_sai_p2p_internal_id", label: "Internal Id"}),
               search.createColumn({name: "custrecord_sai_p2p_transaction_type", label: "Transaction Type"})
            ]
         });
    ...
    function map(context) {
        try{
            ...
            let externalId = result.values['custrecord_sai_p2p_external_id'];
            let attachmentFileId = result.values['custrecord_sai_p2p_file_attachment'].value;
            let tranType = result.values['custrecord_sai_p2p_transaction_type'];
                ...
                if (searchResult.length > 0) {
                    ...
                    try {
                        ...
                        record.submitFields({
                            ...
                            id: internalId,
                            values: {
                                custrecord_sai_p2p_file_attached: true,
                                custrecord_sai_p2p_related_transaction: purchaseOrderId
                            },
                            options: {
                                enableSourcing: true,
