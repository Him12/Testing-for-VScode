...
define(['N/search', 'N/file', 'N/record', 'N/log', 'N/runtime', 'N/email'], (search, file, record, log, runtime, email) => {
    ...
    const map = (context) => {
        ...
        try {
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                if (!lineObj['Date'] || !lineObj['Hours'] || !lineObj['Service Item']) {
    ...
    const reduce = (context) => {
        ...
        try {
            ...
            const approvalStatus = entry['ApprovalStatus'] ? getApprovalValue(entry['ApprovalStatus']) : 2;
            ...
            /* if (entry['Service Item']) {
              ...
              if (itemId) {
                timeRec.setValue({ fieldId: 'item', value: itemId });
              } else {
                throw new Error(`Invalid Service Item: No internal ID found for "${entry['Service Item']}"`);
              }
            } */
