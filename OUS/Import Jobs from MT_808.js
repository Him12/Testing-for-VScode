...
define(['N/record', "N/search", "N/format", "./ous_job_helper", "moment", "dmHelper", "sublistHelper", "errorHelper", "searchHelper", "formSelector", "emailHelper", "cHelper", "lodash"],
    function (record, search, format, jobHelper, moment, dmHelper, suHelper, eHelper, sHelper, fh, emailHelper, cHelper, lodash)
    {
        ...
        function getCustomerAndSubFromJson(customerId)
        {
            ...
            return {customer: customer, subsidiary: subsidiary, paymentTerms : paymentTerms};
        }
        ...
        {
            var subsidiary = "", territory = "", serviceCode = "", entityStatus = "", customer = "";
        ...
        function getEmployee(employeeNo)
        {
            ...
            return {id: null, name: null, employeeNo: null};
        }
        ...
        function addJobEstimates(jobMT, jobId, exists)
        {
            ...
            if (!exists) //Create new estimate
            {
                jobEstimate = record.create({
                    type: record.Type.ESTIMATE,
                    isDynamic: true,
                    defaultValues: {customform: fh.getFormId("OSMCustomEstimate")}
                });
            }
            addGov(10);
            ...
            catch (error)
            {
                ...
                throw {message: "Employee " + dbd.name + " is not a valid DBD."};
            }
            ...
            if (liItemsDontExists)
            {
                throw {message: liItemsDontExists};
            }
        ...
        function addJobResource(job, sublistId, jobResourceId, role, name)
        {
            ...
            try
            {
                ...
                job.commitLine({sublistId: sublistId});
            }
            catch (e)
            {
        ...
        function addJobResourceRoles(jobMT, job)
        {
            ...
            jobResources.push({role: "5", id: getEmployee(jobMT[g_contrAdmin]).id});
            jobResources.push({role: "4", id: getEmployee(jobMT[g_projEst]).id});
            jobResources.push({role: "3", id: getEmployee(jobMT[g_dbd]).id});
            jobResources.push({role: "1", id: getEmployee(jobMT[g_genMan]).id});
            jobResources.push({role: "2", id: getEmployee(jobMT[g_manOpProjLead]).id});
            ...
            for (var i in employeeJSON)
            {
                ...
                var obj = {id: emp.id, name: emp.name};
            ...
            for (var i = count - 1; i >= 0; i--)
            {
                ...
                var jobResc = lodash.find(jobResources, {role: role, id: id});
        ...
        {
            var columns = [{name: "isinactive", sort: "DESC"}, {name: "internalid", sort: "ASC"}, "name"];
                ...
                if (projJob)
                {
                    ...
                    if (projId != projJob["custrecord_osm_project_id"])
                    {
                        record.submitFields({
                            ...
                            id: internalId,
                            values: {custrecord_osm_project_id: projId, name: jobName}
                        });
                        log.debug("1", 1);
        ...
        function createJob(jobs, jobMT)
        {
            ...
            else
            {
                job = record.create({
                    type: record.Type.JOB,
                    isDynamic: true,
                    defaultValues: {customform: customForm}
                });
                g_bExists = false;
            }
            ...
            if (liJobResourceNotValid)
            {
                throw {message: liJobResourceNotValid};
            }
            ...
            record.submitFields({
                type: "job",
                id: jobId,
                values: {custentity_osm_job_contract_id: projIdForProjJob}
            });
        ...
        function rollback(jobId)
        {
            ...
            if (jobId)
            {
                obj.job = {jobId: jobId, jobNumber: g_jobNo}
            }
            if (g_rollbackEstimates.length >= 1)
            {
                obj.Estimates = {estimates: g_rollbackEstimates}
            }
            if (g_rollbackAdditCost.length >= 1)
            {
                obj.AdditionalCost = {"Additional Cost": g_rollbackAdditCost}
            }
            if (g_rollbackOpportunities.length >= 1)
            {
                obj.Opportunities = {"Opportunities to remove job from": g_rollbackOpportunities}
            }
        ...
        function _post(jobs)
        {
            ...
            var objSuccess = Object();
            objSuccess.FailureCount = 0;
            objSuccess.SuccessCount = 0;
            objSuccess.OverallCount = 0;
            objSuccess.Errors = [];
            objSuccess.FailedData = [];
            objSuccess.Governance = 0;
            ...
            try
            {
                ...
                {
                    // emailHelper.sendEmail("job json data", JSON.stringify(jobs), emailHelper.get.DEVELOPER(), true);
            ...
            catch (e)
            {
                ...
                log.error("error", eHelper.getError(e));
                return eHelper.getError(e);
            }

            var job;
            // cHelper.logTimeDiff(dateTime, "setup");
            for (var i in jobs)
            {
                ...
                // log.debug("estimate", job["JobEstimates"]);
                var nextGov = 15;
                ...
                var deleteGov = 5 + calculateGov(5, job["AdditionalJobInfo"]) + calculateGov(5, job["JobEstimates"]) + calculateGov(30, job["JobEstimates"]);

                var jobId = "";
                // if (job["Job Name"].toLowerCase().indexOf("Pedernales Elec Coop.C") != -1)
                // {
                try
                {
                    nextGov += calculateGov(11, job["AdditionalJobInfo"]);  //additional cost governance for this job.
                    nextGov += calculateGov(4, job["JobEstimates"]);        //job estimate governance for this job.
                    nextGov += calculateGov(30);                            //adding job to opportunity governance for this job.
                    ...
                    {
                        dateTime = new Date();
                        //sending jobs for project search reasons
                        jobId = createJob(jobs, job);

                        // cHelper.logTimeDiff(dateTime, "create job total time");
                        objSuccess.SuccessCount += 1;
                    }
                    else
                    {
                        throw "You have exceeded the governance allowed for this call.";
                    }
                }
                catch (e)
                {
                    ...
                    objSuccess.FailedData.push(job);
                    ...
                    if (jobId)
                    {
                        try
                        {
                            ...
                            else
                            {
                                try
                                {
                                    ...
                                    // objSuccess.Errors.push({message: "Unable to roll back changes!", id: g_jobNo});
                                }
                                catch (eb)
                                {
                                }
                        ...
                        {
                            objSuccess.Errors.push(eHelper.getError(ex));
                        }
                    }
                }
            ...
            // log.debug("failed data", objSuccess.FailedData);
            return objSuccess;
        }
