 ...
 * Description: This script sends email attachments (CSV or PDF) from the 'Netstock' folder to predefined recipients in bulk. It checks for file modifications and sends updated files only, tracking email sends in custom records with file names and types.
 *
 * Version History:
...
define(['N/email', 'N/file', 'N/record', 'N/search', 'N/runtime'], function(email, file, record, search, runtime) {
    ...
    function execute(context) {
        try {
            ...
            // Send bulk email if there are modified files
            if (modifiedFiles.length > 0) {
                var recipients = ['Kenny.garner@rhinorack.com.au', 'Sristi.saha-prasad@rhinorack.com.au'];
                var emailSubject = 'Updated NETSTOCK Files (' + modifiedFiles.length + ' files)';
                var emailBody = 'Hi,\n\nAttached are ' + modifiedFiles.length + ' updated files from the NETSTOCK folder.\n\nFiles included:\n';
                
                modifiedFiles.forEach(function(file) {
                    emailBody += '- ' + file.name + ' (Type: ' + file.fileType + ')\n';
                });
                ...
                log.audit('Email Details', JSON.stringify({
                    ...
                    fileNames: modifiedFiles.map(function(file) { return file.name; }),
                    fileTypes: modifiedFiles.map(function(file) { return file.fileType; })
                }, null, 2));
                ...
                // Update custom records for modified files only
                ...
                log.debug('Bulk Email Sent', 'Email sent successfully with ' + modifiedFiles.length + ' files');
            } else {
